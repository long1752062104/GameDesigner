...
namespace ILRuntime.Runtime.Enviorment
{
    public class DelegateManager
    {
        ...
        internal Delegate ConvertToDelegate(Type clrDelegateType, IDelegateAdapter adapter)
        {
            ...
            if (clrDelegates.TryGetValue(clrDelegateType, out Func<Delegate, Delegate> func))
            {
            ...
            else
            {
                ...
                clrDelegateType.GetClassName(out string clsName, out string rName, out bool isByRef);
                ...
                sb.Append(rName);
                sb.Append("((");
                System.Reflection.MethodInfo mi = clrDelegateType.GetMethod("Invoke");
                bool first = true;
                foreach (System.Reflection.ParameterInfo i in mi.GetParameters())
                {
                    if (first)
                    {
                        first = false;
        ...
        internal IDelegateAdapter FindDelegateAdapter(ILTypeInstance instance, ILMethod ilMethod, IMethod method)
        {
            ...
            if (method.ReturnType == appdomain.VoidType)
            {
                ...
                foreach (DelegateMapNode i in methods)
                {
            ...
            {
                foreach (DelegateMapNode i in functions)
                {
