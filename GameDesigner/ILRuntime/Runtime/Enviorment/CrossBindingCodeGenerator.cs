...
namespace ILRuntime.Runtime.Enviorment
{
    public class CrossBindingCodeGenerator
    {
        ...
        public static string GenerateCrossBindingAdapterCode(Type baseType, string nameSpace)
        {
            ...
            baseType.GetClassName(out string clsName, out string realClsName, out bool isByRef, true);
...
");
        ...
        static void GenerateCrossBindingMethodBody(StringBuilder sb, List<MethodInfo> virtMethods)
        {
            ...
            foreach (MethodInfo i in virtMethods)
            {
                if (ShouldSkip(i))
                    continue;
                ...
                if (isProperty)
                {
                    ...
                    if (i.Name == "get_Item" || i.Name == "set_Item")
                    {
                        ...
                        ParameterInfo p = i.GetParameters()[0];
            ...
            foreach (KeyValuePair<string, PropertyGenerateInfo> i in pendingProperties)
            {
                PropertyGenerateInfo pInfo = i.Value;
                pInfo.ReturnType.GetClassName(out string clsName, out string realClsName, out bool isByRef, true);
        ...
        static string GetParametersString(ParameterInfo[] param, Type returnType)
        {
            StringBuilder sb = new StringBuilder();
            bool first = true;
            foreach (ParameterInfo i in param)
            {
                if (!first)
                    sb.Append(", ");
                else
                    first = false;
                i.ParameterType.GetClassName(out string clsName, out string realClsName, out bool isByRef, true);
                sb.Append(realClsName);
            }
            if (returnType != typeof(void))
            {
                if (!first)
                    sb.Append(", ");
                returnType.GetClassName(out string clsName, out string realClsName, out bool isByRef, true);
                sb.Append(realClsName);
            }
            return sb.ToString();
        }
        ...
        static void GetParameterDefinition(StringBuilder sb, ParameterInfo[] param, bool first)
        {
            ...
            foreach (ParameterInfo p in param)
            {
                if (!first)
                    ...
                    first = false;
                p.ParameterType.GetClassName(out string clsName, out string realClsName, out bool isByRef, true);
                if (p.IsOut)
                    sb.Append("out ");
                else if (isByRef)
                    sb.Append("ref ");
