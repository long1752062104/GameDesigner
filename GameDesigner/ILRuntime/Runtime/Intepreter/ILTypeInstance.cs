...
namespace ILRuntime.Runtime.Intepreter
{
    public class ILTypeStaticInstance : ILTypeInstance
    {
        public unsafe ILTypeStaticInstance(ILType type)
        {
            ...
            {
                IType ft = type.StaticFieldTypes[i];
                Type t = ft.TypeForCLR;
                managedObjs.Add(null);
                StackObject.Initialized(ref fields[i], i, t, ft, managedObjs);
            }
            int idx = 0;
            foreach (Mono.Cecil.FieldDefinition i in type.TypeDefinition.Fields)
            {
                if (i.IsStatic)
                {
    ...
    unsafe class ILEnumTypeInstance : ILTypeInstance
    {
        ...
        {
            Mono.Collections.Generic.Collection<Mono.Cecil.FieldDefinition> fields = type.TypeDefinition.Fields;
            long longVal = 0;
            int intVal = 0;
    ...
    public class ILTypeInstance
    {
        ...
        public unsafe int GetSizeInMemory(HashSet<object> traversedObj)
        {
            ...
            int size = SizeOfILTypeInstance + sizeof(StackObject) * fields.Length;
            if (managedObjs != null)
            {
                size += managedObjs.Count * 4;
                foreach (object i in managedObjs)
                {
        ...
        static int GetSizeInMemory(object obj, HashSet<object> traversedObj)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (collection != null)
                    {
                        System.Collections.IEnumerator enu = collection.GetEnumerator();
                        while (enu.MoveNext())
                        {
                    ...
                    else
                    {
                        ...
                        if (dictionary != null)
                        {
                            System.Collections.IDictionaryEnumerator enu = dictionary.GetEnumerator();
                            while (enu.MoveNext())
                            {
        ...
        internal unsafe void PushToStack(int fieldIdx, StackObject* esp, ILIntepreter intp, IList<object> managedStack)
        {
            ...
            else
            {
                if (Type.FirstCLRBaseType != null && Type.FirstCLRBaseType is Enviorment.CrossBindingAdaptor)
                {
                    ...
                    object obj = clrType.GetFieldValue(fieldIdx, clrInstance);
        ...
        unsafe void PushToStackSub(ref StackObject field, int fieldIdx, StackObject* esp, IList<object> managedStack, ILIntepreter intp)
        {
            if (field.ObjectType >= ObjectTypes.Object)
            {
                ...
                {
                    Type ot = obj.GetType();
                    if (ot.IsValueType && type.AppDomain.ValueTypeBinders.TryGetValue(ot, out ValueTypeBinder binder))
                    {
                    ...
                    case ObjectTypes.ValueTypeObjectReference:
                        {
                            ...
                            StackObject* dst = ILIntepreter.ResolveReference(val);
                            IType vt = type.AppDomain.GetType(dst->Value);
                            if (vt is ILType)
                            {
        ...
        internal void InitializeField(int fieldIdx)
        {
            ...
            while (curType != null)
            {
                ...
                {
                    IType ft = curType.FieldTypes[fieldIdx - curType.FieldStartIndex];
        ...
        internal unsafe void AssignFromStack(int fieldIdx, StackObject* esp, Enviorment.AppDomain appdomain, IList<object> managedStack)
        {
            ...
            else
            {
                if (Type.FirstCLRBaseType != null && Type.FirstCLRBaseType is Enviorment.CrossBindingAdaptor)
                {
                    ...
                    System.Reflection.FieldInfo field = clrType.GetField(fieldIdx);
        ...
        internal unsafe void AssignFromStack(StackObject* esp, Enviorment.AppDomain appdomain, IList<object> managedStack)
        {
            ...
            for (int i = 0; i < cnt; i++)
            {
                StackObject* addr = ILIntepreter.Minus(val, i + 1);
        ...
        unsafe void AssignFromStackSub(ref StackObject field, int fieldIdx, StackObject* esp, IList<object> managedStack)
        {
            ...
            switch (field.ObjectType)
            {
                ...
                case ObjectTypes.ValueTypeObjectReference:
                    {
                        Enviorment.AppDomain domain = type.AppDomain;
                        field.ObjectType = ObjectTypes.Object;
                        field.Value = fieldIdx;
                        StackObject* dst = ILIntepreter.ResolveReference(esp);
                        ...
                        if (vt is ILType)
                        {
                            object ins = managedObjs[fieldIdx];
                            if (ins == null)
        ...
        public override string ToString()
        {
            IMethod m = type.ToStringMethod;
            if (m != null)
            {
                if (m is ILMethod)
                {
                    object res = type.AppDomain.Invoke(m, this, null);
                    return res.ToString();
                }
                else
        ...
        public override bool Equals(object obj)
        {
            if (type != null)
            {
                IMethod m = type.EqualsMethod;
                if (m != null && m is ILMethod)
                {
                    using (InvocationContext ctx = type.AppDomain.BeginInvoke(m))
                    {
                        ctx.PushObject(this);
                        ctx.PushObject(obj);
                        ctx.Invoke();
                ...
                else
                {
                    if (this is ILEnumTypeInstance)
                    {
                        if (obj is ILEnumTypeInstance)
                        {
                            ...
                            {
                                bool res = enum1.fields[0] == enum2.fields[0];
                                return res;
                            }
                            else
                                return false;
        ...
        public override int GetHashCode()
        {
            if (type != null)
            {
                IMethod m = type.GetHashCodeMethod;
                if (m != null && m is ILMethod)
                {
                    using (InvocationContext ctx = type.AppDomain.BeginInvoke(m))
                    {
        ...
        internal IDelegateAdapter GetDelegateAdapter(ILMethod method)
        {
            ...
            if (delegates.TryGetValue(method, out IDelegateAdapter res))
                return res;
            return null;
        }
