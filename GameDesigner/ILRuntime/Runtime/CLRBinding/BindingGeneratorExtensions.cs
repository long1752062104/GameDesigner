    ...
    static class BindingGeneratorExtensions
    {
        ...
        internal static bool ShouldSkipMethod(this Type type, MethodBase i)
        {
            ...
            if (i.IsSpecialName)
            {
                ...
                {
                    Type[] ts;
                    int cnt = t[0] == "set" ? param.Length - 1 : param.Length;
                    ...
                    if (cnt > 0)
                    {
                        ...
                        ts = new Type[0];
                    PropertyInfo prop = type.GetProperty(t[1], ts);
                    if (prop == null)
                    {
                        return true;
                    }
        ...
        {
            p.GetClassName(out string clsName, out string realClsName, out bool isByRef);
            Type pt = p.IsByRef ? p.GetElementType() : p;
