    ...
    static class MethodBindingGenerator
    {
        static string GenerateParameterTypes(MethodInfo i, bool needBracket)
        {
            ...
            if (needBracket)
                sb2.Append("{");
            bool first = true;
            foreach (ParameterInfo j in param)
            {
                if (first)
                    first = false;
                else
                    sb2.Append(", ");
                sb2.Append("typeof(");
                j.ParameterType.GetClassName(out string clsName, out string realClsName, out bool isByRef);
        ...
        internal static string GenerateMethodRegisterCode(this Type type, MethodInfo[] methods, HashSet<MethodBase> excludes, out bool needMethods)
        {
            ...
            foreach(var m in type.GetMethods())
            {
                if(m.IsGenericMethodDefinition)
                {
                    ...
                    Type[] param = i.GetGenericArguments();
                    StringBuilder sb2 = new StringBuilder();
                    sb2.Append("{");
                    bool first = true;
                    string clsName, realClsName;
                    bool isByRef;
                    foreach (Type j in param)
                    {
                        if (first)
                            first = false;
                        else
                ...
                else
                {
                    ...
                    i.ReturnType.GetClassName(out string clsName, out string realClsName, out bool isByRef);
        ...
        internal static string GenerateMethodWraperCode(this Type type, MethodInfo[] methods, string typeClsName, HashSet<MethodBase> excludes, List<Type> valueTypeBinders, Enviorment.AppDomain domain)
        {
            ...
            foreach (MethodInfo i in methods)
            {
                ...
                if (!i.IsStatic)
                {
                    ...
                    {
                        type.GetClassName(out string clsName, out string realClsName, out bool isByRef);
                ...
                if (i.IsGenericMethod)
                {
                    ...
                    foreach (Type j in p)
                    {
                        if (first)
                            ...
                            sb2.Append(", ");
                        j.GetClassName(out string clsName, out string realClsName, out bool isByRef);
                        sb2.Append(realClsName);
                    }
                    sb2.Append('>');
                ...
                if (i.IsStatic)
                {
                    if (isProperty)
                    {
                        ...
                        {
                            i.DeclaringType.GetClassName(out string clsName, out string realClsName, out bool isByRef);
                ...
                for (int j = param.Length; j > 0; j--)
                {
                    ...
                    pt.GetClassName(out string clsName, out string realClsName, out bool isByRef);
                    sb.AppendLine(string.Format("            ptr_of_this_method = ILIntepreter.Minus(__esp, {0});", param.Length - j + 1));
