...
namespace ILRuntime.Runtime.CLRBinding
{
    static class CommonBindingGenerator
    {
        internal static string GenerateMiscRegisterCode(this Type type, string typeClsName, bool defaultCtor, bool newArr)
        {
            ...
            {
                BindingFlags constructorFlags = BindingFlags.Public | BindingFlags.Instance | BindingFlags.Static | BindingFlags.DeclaredOnly;
                bool hasDefaultConstructor = type.GetConstructor(constructorFlags, null, new Type[0], null) != null;
            ...
            if (newArr)
            {
                if (!type.IsAbstract || !type.IsSealed)
                {
                    if (type.IsArray)
                    {
                        ...
                        elementType.GetClassName(out string clsName, out string elem, out bool isByRef);
                        string trail = "";
