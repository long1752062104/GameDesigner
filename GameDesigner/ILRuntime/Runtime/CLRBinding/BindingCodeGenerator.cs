                                               ...
                                               List<Type> valueTypeBinders = null, List<Type> delegateTypes = null)
        {
            ...
            foreach (Type i in types)
            {
                if (i.GetCustomAttributes(typeof(ObsoleteAttribute), true).Length > 0)
                    continue;
                i.GetClassName(out string clsName, out string realClsName, out bool isByRef);
...
");
                    ...
                    string registerMethodCode = i.GenerateMethodRegisterCode(methods, excludeMethods, out bool needMethods);
            ...
            List<string> delegateClsNames = GenerateDelegateBinding(delegateTypes, outputPath);
                                               ...
                                               params string[] excludeFiles)
        {
            ...
            foreach (KeyValuePair<Type, CLRBindingGenerateInfo> info in infos)
            {
...
");
                    ...
                    if (info.Value.ValueTypeNeeded)
                    {
                        ...
                        FieldInfo[] fs = i.GetFields(BindingFlags.Public | BindingFlags.Instance | BindingFlags.Static | BindingFlags.DeclaredOnly);
...
namespace ILRuntime.Runtime.Generated
{
    ...
    {");
        ...
        {
            CLR.TypeSystem.IType[] arr = domain.LoadedTypes.Values.ToArray();
            //Prewarm
            foreach (CLR.TypeSystem.IType type in arr)
            {
                if (type is CLR.TypeSystem.ILType)
                {
                    if (type.HasGenericParameter)
                        continue;
                    List<CLR.Method.IMethod> methods = type.GetMethods().ToList();
                    foreach (CLR.Method.ILMethod i in ((CLR.TypeSystem.ILType)type).GetConstructors())
                        methods.Add(i);
                    if (((CLR.TypeSystem.ILType)type).GetStaticConstroctor() != null)
                        methods.Add(((CLR.TypeSystem.ILType)type).GetStaticConstroctor());
                    foreach (CLR.Method.IMethod j in methods)
                    {
                        ...
                        if (method != null)
                        {
                            if (method.GenericParameterCount > 0 && !method.IsGenericInstance)
                                continue;
                            Intepreter.OpCodes.OpCode[] body = method.Body;
                        }
                    }
                }
            }
                    ...
                    foreach (CLR.Method.IMethod j in methods)
                    {
                        ...
                        if (method != null)
                        {
                            if (method.GenericParameterCount > 0 && !method.IsGenericInstance)
                                continue;
                            Intepreter.OpCodes.OpCode[] body = method.Body;
                            foreach (Intepreter.OpCodes.OpCode ins in body)
                            {
                                switch (ins.Code)
                                {
                                    case Intepreter.OpCodes.OpCodeEnum.Newobj:
                                        {
                                            ...
                                            if (m != null)
                                            {
                                                ...
                                                if (!infos.TryGetValue(t, out CLRBindingGenerateInfo info))
                                                {
                                    ...
                                    case Intepreter.OpCodes.OpCodeEnum.Stsfld:
                                        {
                                            CLR.TypeSystem.CLRType t = domain.GetType((int)(ins.TokenLong >> 32)) as CLR.TypeSystem.CLRType;
                                            if (t != null)
                                            {
                                                FieldInfo fi = t.GetField((int)ins.TokenLong);
                                                if (fi != null && fi.IsPublic)
                                                {
                                                    if (!infos.TryGetValue(t.TypeForCLR, out CLRBindingGenerateInfo info))
                                                    {
                                    ...
                                    case Intepreter.OpCodes.OpCodeEnum.Newarr:
                                        {
                                            CLR.TypeSystem.CLRType t = domain.GetType(ins.TokenInteger) as CLR.TypeSystem.CLRType;
                                            if (t != null)
                                            {
                                                if (!infos.TryGetValue(t.TypeForCLR, out CLRBindingGenerateInfo info))
                                                {
    ...
    unsafe class ");
        ...
        {");
                    ...
                    if (mi.ReturnType != typeof(void))
                    {
                        ...
                        first = true;
                        foreach (ParameterInfo j in miParameters)
                        {
                            if (first)
                            {
                                first = false;
...
");
                ...
                if (valueTypeBinders != null)
                {
                    ...
                    foreach (Type i in valueTypeBinders)
                    {
                        i.GetClassName(out string clsName, out string realClsName, out bool isByRef);
