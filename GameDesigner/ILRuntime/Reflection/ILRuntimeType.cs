...
namespace ILRuntime.Reflection
{
    ...
    {
        ILType type;
        internal Runtime.Enviorment.AppDomain appdomain;
        ...
        void InitializeCustomAttribute()
        {
            ...
            {
                Mono.Cecil.CustomAttribute attribute = type.TypeDefinition.CustomAttributes[i];
                IType at = appdomain.GetType(attribute.AttributeType, type, null);
                try
                {
        ...
        void InitializeFields()
        {
            ...
            {
                IType t = type.GetField(i, out Mono.Cecil.FieldDefinition fd);
        ...
        {
            List<ILMethod> ctors = type.GetConstructors();
        ...
        public override bool IsInstanceOfType(object o)
        {
            ...
            ILTypeInstance instance = o as ILTypeInstance;
        ...
        public override FieldInfo GetField(string name, BindingFlags bindingAttr)
        {
            if (fields == null)
                InitializeFields();
            foreach (ILRuntimeFieldInfo i in fields)
            {
                if (i.Name == name)
                    return i;
            }
        ...
        public override Type[] GetInterfaces()
        {
            if (type.Implements == null)
                return new Type[0];
            Type[] interfaces = new Type[type.Implements.Length];
            for (int i = 0, length = type.Implements.Length; i < length; i++)
            {
                IType t = type.Implements[i];
                if (t != null)
        ...
        public override PropertyInfo[] GetProperties(BindingFlags bindingAttr)
        {
            ...
            foreach (ILRuntimePropertyInfo i in properties)
            {
                if (isPublic != i.IsPublic && isPrivate != !i.IsPublic)
                    continue;
                if ((isStatic != i.IsStatic) && (isInstance != !i.IsStatic))
        ...
        protected override ConstructorInfo GetConstructorImpl(BindingFlags bindingAttr, Binder binder, CallingConventions callConvention, Type[] types, ParameterModifier[] modifiers)
        {
            ...
            for (int i = 0; i < types.Length; i++)
            {
                ...
                {
                    IType t = appdomain.GetType(types[i]);
                    if (t == null)
            ...
            IMethod res = type.GetConstructor(param);
        ...
        {
            KeyValuePair<string, IType>[] args = type.GenericArguments;
        ...
        {
            ILType def = type.GetGenericDefinition();
        ...
        {
            return 1;//让ilr支持pb序列化热更新自定义类型的数组字段
        }
    }
}
