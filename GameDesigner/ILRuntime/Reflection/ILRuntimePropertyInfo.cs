...
namespace ILRuntime.Reflection
{
    public class ILRuntimePropertyInfo : PropertyInfo
    {
        ...
        {
            int indexCnt = index != null ? index.Length : 0;
            if (getter.ParameterCount <= indexCnt)
            {
                using (Runtime.Enviorment.InvocationContext ctx = appdomain.BeginInvoke(getter))
                {
                    if (!IsStatic)
                        ctx.PushObject(obj);
        ...
        public override void SetValue(object obj, object value, BindingFlags invokeAttr, Binder binder, object[] index, CultureInfo culture)
        {
            ...
            {
                using (Runtime.Enviorment.InvocationContext ctx = appdomain.BeginInvoke(setter))
                {
                    if (!IsStatic)
                        ctx.PushObject(obj);
