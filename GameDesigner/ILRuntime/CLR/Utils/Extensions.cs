    ...
    public static class Extensions
    {
        ...
        public static List<IType> GetParamList(this MethodReference def, ILRuntime.Runtime.Enviorment.AppDomain appdomain, IType contextType, IMethod contextMethod, IType[] genericArguments)
        {
            if (def.HasParameters)
            {
                ...
                IType dt = appdomain.GetType(def.DeclaringType, contextType, contextMethod);
                foreach (ParameterDefinition i in def.Parameters)
                {
                    ...
                    if ((t == null && def.IsGenericInstance) || (t != null && t.HasGenericParameter))
                    {
                        ...
                        {
                            GenericParameter gp = gim.ElementMethod.GenericParameters[j];
                            TypeReference ga = gim.GenericArguments[j];
                            if (name == gp.Name)
                            {
                        ...
                        {
                            foreach (KeyValuePair<string, IType> gp in dt.GenericArguments)
                            {
        ...
        static string ReplaceGenericArgument(string typename, string argumentName, string argumentType, bool isGA = false)
        {
            ...
            Runtime.Enviorment.AppDomain.ParseGenericType(typename, out string baseType, out List<string> ga, out bool isArray, out byte rank);
        ...
        {
            TypeFlags result = TypeFlags.Default;
        ...
        public static object CheckCLRTypes(this Type pt, object obj)
        {
            ...
            TypeFlags typeFlags = GetTypeFlags(pt);
            ...
            {
                IDelegateAdapter adapter = obj as IDelegateAdapter;
        ...
        {
            ParameterInfo[] arr = m.GetParameters();
            if (arr.Length != args.Length) return false;
            for (int i = 0; i < args.Length; i++)
            {
                Type t1 = arr[i].ParameterType;
                Type t2 = args[i];
