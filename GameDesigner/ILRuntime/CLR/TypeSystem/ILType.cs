...
namespace ILRuntime.CLR.TypeSystem
{
    public class ILType : IType
    {
        ...
        public List<IMethod> GetMethods()
        {
            ...
            foreach (KeyValuePair<string, List<ILMethod>> i in methods)
            {
                foreach (ILMethod j in i.Value)
                    res.Add(j);
            }
        ...
        void InitializeInterfaces()
        {
            ...
            if (definition != null && definition.HasInterfaces)
            {
                ...
                for (int i = 0; i < interfaces.Length; i++)
                {
                    ...
                    {
                        if (appdomain.CrossBindingAdaptors.TryGetValue(interfaces[i].TypeForCLR, out CrossBindingAdaptor adaptor))
                        {
        ...
        void InitializeBaseType()
        {
            if (definition != null && definition.BaseType != null)
            {
                ...
                if (definition.BaseType.IsGenericInstance)
                {
                    ...
                    IType elementType = appdomain.GetType(git.ElementType, this, null);
                    if (elementType is CLRType)
                    {
                        for (int i = 0; i < git.GenericArguments.Count; i++)
                        {
                            TypeReference ga = git.GenericArguments[i];
                            if (ga == typeRef)
                            {
                                specialProcess = true;
                                if (spIdx == null)
                ...
                if (specialProcess)
                {
                    ...
                    IType elementType = appdomain.GetType(((GenericInstanceType)definition.BaseType).ElementType, this, null);
                    foreach (KeyValuePair<Type, CrossBindingAdaptor> i in appdomain.CrossBindingAdaptors)
                    {
                        if (i.Key.IsGenericType && !i.Key.IsGenericTypeDefinition)
                        {
                            Type gd = i.Key.GetGenericTypeDefinition();
                            if (gd == elementType.TypeForCLR)
                            {
                                Type[] ga = i.Key.GetGenericArguments();
                                bool match = true;
                                foreach (int j in spIdx)
                                {
                ...
                else
                {
                    ...
                    if (baseType is CLRType)
                    {
                        ...
                        {
                            if (appdomain.CrossBindingAdaptors.TryGetValue(baseType.TypeForCLR, out CrossBindingAdaptor adaptor))
                            {
                                baseType = adaptor;
                            }
                            else
        ...
        public IMethod GetMethod(string name)
        {
            if (methods == null)
                InitializeMethods();
            if (methods.TryGetValue(name, out List<ILMethod> lst))
            {
                return lst[0];
            }
            return null;
        ...
        void InitializeMethods()
        {
            ...
            if (definition == null)
                return;
            foreach (MethodDefinition i in definition.Methods)
            {
                if (i.IsConstructor)
                {
                    if (i.IsStatic)
                ...
                {
                    if (!methods.TryGetValue(i.Name, out List<ILMethod> lst))
                    {
                        lst = new List<ILMethod>();
                        methods[i.Name] = lst;
                    }
                    ILMethod m = new ILMethod(i, this, appdomain);
                    lst.Add(m);
                }
            }
        ...
        public IMethod GetVirtualMethod(IMethod method)
        {
            ...
            IMethod m = GetMethod(method.Name, method.Parameters, genericArguments, method.ReturnType, true);
        ...
        public IMethod GetConstructor(int paramCnt)
        {
            if (constructors == null)
                InitializeMethods();
            foreach (ILMethod i in constructors)
            {
        ...
        void InitializeFields()
        {
            ...
            Mono.Collections.Generic.Collection<FieldDefinition> fields = definition.Fields;
        ...
        {
            ILTypeInstance res = new ILTypeInstance(this);
            if (callDefaultConstructor)
            {
                IMethod m = GetConstructor(CLR.Utils.Extensions.EmptyParamList);
                if (m != null)
                {
        ...
        {
            ILTypeInstance res = new ILTypeInstance(this);
            List<IType> argsTypes = new List<IType>(args.Length);
            ...
            IMethod m = GetConstructor(argsTypes);
            if (m != null)
            {
        ...
        public IType MakeGenericInstance(KeyValuePair<string, IType>[] genericArguments)
        {
            ...
            foreach (ILType i in genericInstances)
            {
                bool match = true;
                for (int j = 0; j < genericArguments.Length; j++)
                {
            ...
            ILType res = new ILType(definition, appdomain);
        ...
        public IType MakeArrayType(int rank)
        {
            ...
            {
                ArrayType def = new ArrayType(typeRef, rank);
                atype = new ILType(def, appdomain);
                ((ILType)atype).IsArray = true;
                ((ILType)atype).elementType = this;
                ((ILType)atype).arrayCLRType = rank > 1 ? TypeForCLR.MakeArrayType(rank) : TypeForCLR.MakeArrayType();
        ...
        {
            KeyValuePair<string, IType>[] ga = contextType.GenericArguments;
            if (definition == null)
                return null;
            IType[] kv = new IType[definition.GenericParameters.Count];
            for (int i = 0; i < kv.Length; i++)
            {
                GenericParameter gp = definition.GenericParameters[i];
                string name = gp.Name;
                foreach (KeyValuePair<string, IType> j in ga)
                {
                    if (j.Key == name)
                    {
                        kv[i] = j.Value;
