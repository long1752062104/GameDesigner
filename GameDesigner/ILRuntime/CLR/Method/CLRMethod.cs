...
namespace ILRuntime.CLR.Method
{
    public class CLRMethod : IMethod
    {
        ...
        public unsafe object Invoke(Runtime.Intepreter.ILIntepreter intepreter, StackObject* esp, IList<object> mStack, bool isNewObj = false)
        {
            ...
            {
                StackObject* p = Minus(esp, i);
                Type pt = this.param[paramCount - i].ParameterType;
                object obj = pt.CheckCLRTypes(StackObject.ToObject(p, appdomain, mStack));
        ...
        {
            int cnt = hasThis ? paramCount + 1 : paramCount;
            for (int i = cnt; i >= 1; i--)
            {
                StackObject* p = Minus(esp, i);
                object val = i <= paramCount ? param[paramCount - i] : instance;
                switch (p->ObjectType)
                {
                    case ObjectTypes.StackObjectReference:
                        {
                            long addr = *(long*)&p->Value;
                            StackObject* dst = (StackObject*)addr;
                    ...
                    case ObjectTypes.StaticFieldReference:
                        {
                            IType t = appdomain.GetType(p->Value);
                            if (t is ILType)
                            {
                    ...
                    case ObjectTypes.ArrayReference:
                        {
                            Array arr = mStack[p->Value] as Array;
...
#endif
            CLRMethod res = new CLRMethod(t, declaringType, appdomain);
