...
namespace ILRuntime.Mono.Cecil.Cil
{
    ...
    public sealed class PortablePdbReader : ISymbolReader
    {
        ...
        public bool ProcessDebugHeader(ImageDebugHeader header)
        {
            ...
            Guid module_guid = new Guid(buffer);
    ...
    public sealed class EmbeddedPortablePdbReaderProvider : ISymbolReaderProvider
    {
        ...
        {
            MemoryStream compressed_stream = new MemoryStream(entry.Data);
            BinaryStreamReader reader = new BinaryStreamReader(compressed_stream);
            reader.ReadInt32(); // signature
            int length = reader.ReadInt32();
            MemoryStream decompressed_stream = new MemoryStream(length);
    ...
    public sealed class EmbeddedPortablePdbWriter : ISymbolWriter
    {
        ...
        public ImageDebugHeader GetDebugHeader()
        {
            ...
            BinaryStreamWriter w = new BinaryStreamWriter(data);
            w.WriteByte(0x4d);
            w.WriteByte(0x50);
            w.WriteByte(0x44);
            w.WriteByte(0x42);
            ...
            using (DeflateStream compress_stream = new DeflateStream(data, CompressionMode.Compress, leaveOpen: true))
    ...
    static class PdbGuidMapping
    {
        ...
        {
            if (!guid_language.TryGetValue(guid, out DocumentLanguage language))
        ...
        {
            if (!language_guid.TryGetValue(language, out Guid guid))
                return new Guid();
