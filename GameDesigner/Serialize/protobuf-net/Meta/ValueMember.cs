...
namespace ProtoBuf.Meta
{
    ...
    public class ValueMember
    {
        ...
        internal bool ResolveMapTypes(out Type dictionaryType, out Type keyType, out Type valueType)
        {
            ...
            foreach (Type iType in memberType.GetInterfaces())
            {
#if WINRT || COREFX
                ...
                if (info.IsGenericType && info.GetGenericTypeDefinition() == typeof(IDictionary<,>))
                {
                    ...
                    Type[] typeArgs = iType.GetGenericArguments();
        ...
        private IProtoSerializer BuildSerializer()
        {
            ...
            try
            {
                ...
                MemberInfo member = backingMember ?? originalMember;
                ...
                if (IsMap)
                {
                    ...
                    Type concreteType = defaultType;
                    if (concreteType == null && Helpers.IsClass(memberType))
                    {
                        concreteType = memberType;
                    }
                    IProtoSerializer keySer = TryGetCoreSerializer(model, MapKeyFormat, keyType, out WireType keyWireType, false, false, false, false);
                    if (!AsReference)
                    {
                        AsReference = MetaType.GetAsReferenceDefault(model, valueType);
                    }
                    IProtoSerializer valueSer = TryGetCoreSerializer(model, MapValueFormat, valueType, out WireType valueWireType, AsReference, DynamicType, false, true);

                    ConstructorInfo[] ctors = typeof(MapDecorator<,,>).MakeGenericType(new Type[] { dictionaryType, keyType, valueType }).GetConstructors(
            ...
            {
                Net.Event.NDebug.LogError("序列化错误:" + ex);
                return null;
            }
            finally
            {
