    ...
    public class MetaType : ISerializerProxy
    {
            ...
            foreach (MemberInfo member in foundList)
            {
                ...
                if ((property = member as PropertyInfo) != null)
                {
                    ...
                    if (!property.CanWrite)
                    {
                        ...
                        foreach (MemberInfo fieldMemeber in foundList)
                        {
        ...
        private ValueMember ApplyDefaultBehaviour(bool isEnum, ProtoMemberAttribute normalizedAttribute)
        {
            ...
            if (vm != null)
            {
                ...
                vm.IsMap = ignoreListHandling ? false : vm.ResolveMapTypes(out Type _, out Type _, out Type _);
        ...
        private ValueMember AddField(int fieldNumber, string memberName, Type itemType, Type defaultType, object defaultValue)
        {
            ...
            {
                MemberInfo[] backingMembers = type.GetMember($"<{((PropertyInfo)mi).Name}>k__BackingField", Helpers.IsEnum(type) ? BindingFlags.Static | BindingFlags.Public : BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
        ...
        {
#if UNITY_STANDALONE || UNITY_ANDROID || UNITY_IOS || SERVICE && !FEAT_IKVM && !FX11
        ...
        internal System.Collections.IEnumerable Fields { get { return fields; } }
...
.IsDefined(model.MapType(typeof(FlagsAttribute)), false))
                ...
                else
                {
                    ...
                    foreach (ValueMember field in fieldsArr)
                    {
            ...
            else
            {
                ...
                foreach (ValueMember member in fieldsArr)
                {
                    ...
                    if (member.IsMap)
                    {
                        member.ResolveMapTypes(out Type _, out Type keyType, out Type valueType);

                        string keyTypeName = model.GetSchemaTypeName(keyType, member.MapKeyFormat, false, false, ref imports);
