...
namespace Fleck.Handlers
{
    ...
    {
        public static IHandler Create(WebSocketHttpRequest request, Action<byte[],string> onMessage, Action onClose, Action<byte[]> onBinary, Action<byte[]> onPing, Action<byte[]> onPong)
        {
        ...
        public static void ProcessFrame(FrameType frameType, byte[] data, Action<byte[], string> onMessage, Action onClose, Action<byte[]> onBinary, Action<byte[]> onPing, Action<byte[]> onPong)
        {
            switch (frameType)
            {
            ...
            case FrameType.Pong:
                ...
                break;
            case FrameType.Text:
                onMessage(data, ReadUTF8PayloadData(data));
                break;
            default:
