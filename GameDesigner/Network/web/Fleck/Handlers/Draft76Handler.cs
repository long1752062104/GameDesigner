...
namespace Fleck.Handlers
{
    public static class Draft76Handler
    {
        ...
        public static IHandler Create(WebSocketHttpRequest request, Action<byte[],string> onMessage)
        {
        ...
        public static void ReceiveData(Action<byte[],string> onMessage, List<byte> data)
        {
            while (data.Count > 0)
            {
                if (data[0] != Start)
